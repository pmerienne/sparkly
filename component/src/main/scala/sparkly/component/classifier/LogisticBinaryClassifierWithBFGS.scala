...
import org.apache.spark.mllib.regression.{StreamingLinearAlgorithm, LabeledPoint}
...
class LogisticBinaryClassifierWithBFGS extends Component {
  ...
  override def metadata: ComponentMetadata = ComponentMetadata (
    ...
    description =
      """
        |Binary classifier based on a logistic regression model with BFGS.
      """.stripMargin,
    inputs = Map (
      ...
      "Predictions" -> OutputStreamMetadata(from = Some("Predict"), namedFeatures = Map("Prediction" -> FeatureType.BOOLEAN))
    ), properties = Map (
  ...
  override protected def initStreams(context: Context): Map[String, DStream[Instance]] = {
    ...
    val model = new SparklyStreamingLogisticBinaryClassifierWithBFGS()
...
class SparklyStreamingLogisticBinaryClassifierWithBFGS extends StreamingLinearAlgorithm[LogisticRegressionModel, LogisticRegressionWithLBFGS] with Serializable {
